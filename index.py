...
class Index:
    ...
    def __init__(self, parent_dir: str, index_id: str):
        ...
        if not Kmindex.b_index_exists(self.parent_dir, self.index_id):
            raise NotADirectoryError(f"Index directory for '{self.index_id}' not found in {self.parent_dir}")
    ...
    def matrices_path(self) -> str:
        ...
        return Kmindex.get_matrix_dir(self.path)
    ...
    def get_matrix_path(self, partition: int, is_compressed: bool = False) -> str:
        ...
        """
        return Kmindex.get_matrix_path(self.path, partition, is_compressed)
    ...
    def get_matrix_byte_size(self, partition: int, is_compressed: bool = False) -> int:
        ...
        Returns:
            int: Size in bytes
        """
        return Kmindex.get_bytes_per_matrix(self.path, partition, is_compressed)
    ...
    def check_structure(self) -> bool:
        ...
        """
        return Kmindex.check_index_structure(self.path, self.nb_partitions)
    ...
    def get_property(self, key: str) -> Any:
        ...
        if key not in self._properties:
            raise KeyError(f"Property '{key}' not found. Available properties: {list(self._properties.keys())}")
    ...
    def load_kmtricks_index(self):
        self.import_properties(Kmindex.load_options_file(Kmindex.get_options_path(self.path)))
        # load samples 
        samples = Kmindex.load_fof_file(Kmindex.get_fof_path(self.path))
...
class IndexRegistry:
    ...
    def get_index(self, index_id: str) -> Index:
        ...
        if not self.has_index(index_id):
            raise KeyError(f"Index ID '{index_id}' not found. Available IDs: {self.list_indices()}")
    ...
    def add_index(self, index: Index) -> bool:
        if self.has_index(index.index_id):
            return False
        Kmindex.register_index_in_json(index.path, self.root_path, index.index_id)
        return True
    ...
    def __str__(self) -> str:
        ...
        return f"IndexRegistry(path='{self.root_path}', indices={len(indices)}: {indices})"
