...
function usage ()
{
  ...
  echo "Usage: "
  echo "  ./install.sh [-r str] [-t int] [-j int] [-p str] [-k int] [-i str] [-n] [-h]"
  echo "Options: "
  ...
  echo "  -i <PATH>          -> Path to kmindex source directory {../kmindex}"
...
while getopts "k:r:t:j:p:i:nh" option; do
  case "$option" in
    r)
      mode=${OPTARG}
...
kmindex_build ${mode} ${tests_str} ${tests_run} ${native} ${jopt} ${installp} ${max_ksize} ${kmindex_dir}
