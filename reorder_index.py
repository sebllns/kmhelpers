...
def process_index(source_dir, output_dir, index_id, subsamples_rows):
    ...
    # Copy folder while ignoring the matrices directory
    shutil.copytree(
        source_index_path,
        output_index_dir
    )
    ...
    reorder_output = kmhelpers.BitmatrixShuffle.reorder_index(
        output_register_dir, index_id, subsamples_rows)
    
    if reorder_output['returncode'] == 0:
        ordering_result = kmhelpers.BitmatrixShuffle.parse_ordering_output(
        reorder_output['stdout'])
        kmhelpers.Toolbox.save_to_json_file(
            ordering_result,
            os.path.join(output_index_dir, "ordering.json")
        )
...
def main():
    ...
    parser.add_argument(
        "--output",
        required=True,
        help="Output directory (where to put reordered index)",
    )
    parser.add_argument(
        "--index",
        nargs='+',  # one or more
        required=True,
        help="Folder(s) of the index to reorder",
    )

    parser.add_argument(
        "--subsamples_rows", type=int, default=2000, help="Number of rows to use for subsampling (default: 2000)"
    )
    ...
    for index_id in args.index:
        ...
        process_index(args.input, args.output, index_id, args.subsamples_rows)
